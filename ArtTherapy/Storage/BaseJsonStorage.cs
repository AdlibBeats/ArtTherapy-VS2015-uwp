...
namespace ArtTherapy.Storage
{
    ...
    public class BaseJsonStorage<T> : BaseStorage<T> where T : BaseModel
    {
        ...
        {
            Debug.WriteLine($"DELETE: {fileName}: {Windows.ApplicationModel.Package.Current.InstalledLocation.Path}");
            ...
            return Task.Run(async () =>
            {
                try
                {
                    file = await StorageFile.GetFileFromPathAsync(folder1.Path + @"\" + fileName);
                }
                catch (Exception ex)
                {
        ...
        {
            Debug.WriteLine($"GET: {fileName}: {Windows.ApplicationModel.Package.Current.InstalledLocation.Path}");
        ...
        {
            Debug.WriteLine($"SET: {fileName}: {Windows.ApplicationModel.Package.Current.InstalledLocation.Path}");
